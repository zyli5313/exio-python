...
class AuthenticatedClient(PublicClient):
    def __init__(self, key, b64secret, apiUrl="https://api.sandbox.ex.io/v1", timeout=30):
        super(AuthenticatedClient, self).__init__(apiUrl=apiUrl)
        self.auth = ExioAuth(key, b64secret)
    ...
    def cancelOrder(self, symbol, oid):
        ...
        r = requests.delete(self.url + '/orders/', data=payload, auth=self.auth, timeout=self.timeout)
        # r.raise_for_status()
        return r.json()
    ...
    def getOpenOrders(self, symbol):
        ...
        r = requests.get(self.url + '/orders/', data=payload, auth=self.auth, timeout=self.timeout)
        # r.raise_for_status()
        return r.json()
    ...
    def getTradeHistory(self, symbol, beginTime, endTime):
        ...
        r = requests.get(self.url + '/trade_history', auth=self.auth, timeout=self.timeout)
        # r.raise_for_status()
        return r.json()
